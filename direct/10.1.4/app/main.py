...
@app.get("/api/health")
def health():
    return {"ok":True,"interval":settings.interval_minutes,"alert_tolerance_tl":float(db.runtime_all().get("alert_tolerance_tl") or 1300),
...
def alert_tolerance_get():
    return {"ok":True,"value":float(db.runtime_all().get("alert_tolerance_tl") or 1300)}
