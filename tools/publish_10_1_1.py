...
ROOT=Path.cwd(); SRC=ROOT/'_1010.zip'; WORK=ROOT/'_1011'
...
# Filter common-screen results to only selected marketplaces.
needle='    products=db.list_products()\n    checks=db.list_checks()\n'
...
idx=s.find('@app.get("/api/operations/overview")')
if idx<0:raise SystemExit('overview endpoint missing')
pos=s.find(needle,idx)
if pos<0:raise SystemExit('overview body missing')
s=s[:pos]+s[pos:].replace(needle,insert,1)

# Replace save/reset functions by function-boundary regex, regardless whitespace.
start=s.find('def price_protection_markets_save(')
end=s.find('@app.get("/api/update/check")',start)
if start<0 or end<0:raise SystemExit('market selection API boundaries missing')
prefix=s[:start];suffix=s[end:]
...
s=prefix+block+suffix;p.write_text(s,encoding='utf-8')
...
for rel in ['app/main.py','app/static/app.js']:
    ...
    files.append({'path':rel,'url':BASE+'direct/10.1.1/'+rel,'sha256':hashlib.sha256(src.read_bytes()).hexdigest()})
(ROOT/'channel.json').write_text(json.dumps({'version':'10.1.1','published_at':'2026-08-14','notes':'Kritik kayıtları sıfırlama, ortak ekran yenileme ve kontrol dışı pazaryerlerinin eski kayıtlarını temizleme.','files':files},ensure_ascii=False,indent=2),encoding='utf-8')
print('10.1.1 direct files ready')
